...
const CoursesTable = ({title,courseList=[]}) => {
  ...
  const handlePostCourse=()=>{
    ...
    // }
     ...
     const handleDeleteCourse = async (id, courPublicId, caracterePublicId) => {
  try {
    ...
    if (courPublicId) {
      await fetch("/api/delete-file", {
        ...
        body: JSON.stringify({ publicId: courPublicId }),
      });
    }
  ...
  useEffect(() => {
    ...
    setCourses([...courseList]); // Copie par valeur
  }, [courseList]); // met à jour si les props changent
  ...
  return (
    <div className="container-fluid py-4" style={{ margin:"100px 5px 5px 5px" ,direction: 'rtl', fontFamily: 'Arial, sans-serif' }}>
      ...
      <button onClick={handlePostCourse} type="button" className="btn btn-success"><i className="bi bi-plus"></i>Add Course</button>
        </div>): <></>
                    ...
                    {filteredCourses.map((course, index) => (
                      ...
                      onMouseLeave={(e) => {
                        ...
                        <td className="text-center py-4">
                          <button
                            ...
                            onClick={() => window.open(course.courUrl, '_blank')}
                            onMouseOver={(e) => {
                        ...
                        </td>
                      {
                        currentUser? (<td className="text-center py-4">
                           {/* <button type="button" className="btn btn-primary btn-sm m-2" onClick={()=>handleUpdateCourse(course.id)}><i className="bi bi-pen"></i></button> */}
                        <button type="button" className="btn btn-danger btn-sm" onClick={()=>handleDeleteCourse(course.id,course.courPublicId, course.caracterePublicId)}><i className="bi bi-archive-fill"></i></button>
          ...
          <div className="card border-0 shadow-sm" style={{ 
            ...
            <div className="card-body text-white text-center py-3">
              <h4 className="fw-bold mb-0">
                📊 إجمالي الدورات: {courses.length} دورة
              </h4>
            </div>
          </div>
        </div>
