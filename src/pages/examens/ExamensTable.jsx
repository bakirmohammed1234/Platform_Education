...
const ExamensTable= ({title,examenList=[]}) => {
  ...
  const handlePostExamen=()=>{
    ...
    // }
     ...
     const handleDeleteExamen = async (id, examenPublicId, caracterePublicId) => {
  try {
    ...
    if (examenPublicId) {
      await fetch("/api/index/delete-file", {
        ...
        body: JSON.stringify({ publicId: examenPublicId }),
      });
    }
    ...
    setExamens((prev) => prev.filter((examen) => examen.id !== id));

    setSuccessMessage("✅ تم حذف الدورة بنجاح (و الملفات من Cloudinary).");
    clearMessage();
  } catch (error) {
  ...
  useEffect(() => {
     console.log("courseList updated:", examenList);
    setExamens([...examenList]); // Copie par valeur
  }, [examenList]); // met à jour si les props changent
  ...
  //     id: 8,
  //     title: 'علم الأحياء',
  //     pdfLink: '/pdfs/biology.pdf',
  //     createdDate: '2024-04-15'
  //   }
  // ];

  // تصفية الدورات بناءً على البحث
  const filteredExamens = examens.filter(course => 
  ...
  return (
    <div className="container-fluid py-4" style={{ margin:"100px 5px 5px 5px" ,direction: 'rtl', fontFamily: 'Arial, sans-serif' }}>
      ...
      <button onClick={handlePostExamen} type="button" className="btn btn-success"><i className="bi bi-plus"></i>Add Examen</button>
        </div>): <></>
            ...
            <div className="card-body p-0">
              <div className="table-responsive">
                <table className="table table-hover mb-0" style={{ fontSize: '1.1rem' }}>
                  ...
                  <tbody>
                    {filteredExamens.map((course, index) => (
                        ...
                        </td>
                      {
                        ...
                        <button type="button" className="btn btn-danger btn-sm" onClick={()=>handleDeleteExamen(course.id,course.examenPublicId, course.caracterePublicId)}><i className="bi bi-archive-fill"></i></button>
          ...
          <div className="card border-0 shadow-sm" style={{ 
            ...
            <div className="card-body text-white text-center py-3">
              <h4 className="fw-bold mb-0">
                📊 إجمالي الدورات: {examens.length} دورة
              </h4>
            </div>
          </div>
        </div>
